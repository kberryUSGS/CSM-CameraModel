   ...
   csm::WarningList* warnings) const
{
   ...
   double xl, yl, zl;
   double dxl, dyl, dzl;
   // compute look vector in body-fixed coordinates 
   losToEcf(
   ...
   double&       bodyLookZ) const  // output line-of-sight z coordinate
{
   ...
   // Convert CSM coordinates to USGS coordinates
   double fractionalLine, sampleUSGSFull; 
   convertToUSGSCoordinates(line, sample, fractionalLine, sampleUSGSFull);
   ...
   const std::vector<double>& adj) const
{
   ...
   double correctedLookX = adjustedLookX;
   double correctedLookY = adjustedLookY;
   double correctedLookZ = adjustedLookZ; 
