...
UsgsAstroLsSensorModel::replaceModelState (const std::string &stateString)
{
  ...
  MESSAGE_LOG ("m_imageIdentifier: {} "
               ...
               j["m_nLines"].dump (), j["m_nSamples"].dump (),
  ...
  MESSAGE_LOG ("m_detectorSampleOrigin: {} "
               ...
               j["m_detectorLineOrigin"].dump (), j["m_majorAxis"].dump (),
               j["m_minorAxis"].dump ())
    ...
    const std::string &model_state)
{
  ...
  if (j.find ("m_modelName") != j.end ())
  ...
  else
    {
      csm::Error::ErrorType aErrorType
          = csm::Error::INVALID_SENSOR_MODEL_STATE;
  ...
  if (model_name != _SENSOR_MODEL_NAME)
    {
      csm::Error::ErrorType aErrorType
          = csm::Error::SENSOR_MODEL_NOT_SUPPORTED;
      std::string aMessage = "Sensor model not supported.";
      std::string aFunction
          = "UsgsAstroLsPlugin::getModelNameFromModelState()";
      csm::Error csmErr (aErrorType, aMessage, aFunction);
      throw (csmErr);
    }
  return model_name;
...
UsgsAstroLsSensorModel::getModelState () const
{
  ...
  MESSAGE_LOG ("m_imageIdentifier: {} "
               ...
               "m_nSamples: {} "
               "m_platformFlag: {} ",
               m_imageIdentifier, m_sensorName, m_nLines, m_nSamples,
               m_platformFlag)
  ...
  MESSAGE_LOG ("m_detectorSampleOrigin: {} "
               ...
               "m_majorAxis: {} "
               "m_minorAxis: {} ",
               m_detectorSampleOrigin, m_detectorLineOrigin, m_majorAxis,
               m_minorAxis)
  ...
  MESSAGE_LOG ("m_platformIdentifier: {} "
               ...
               "m_minElevation: {} "
               "m_maxElevation: {} ",
               m_platformIdentifier, m_sensorIdentifier, m_minElevation,
               m_maxElevation)
...
UsgsAstroLsSensorModel::updateState ()
{
  ...
  double fullImageTime
      = m_intTimeStartTimes.back () - m_intTimeStartTimes.front ()
...
UsgsAstroLsSensorModel::imageToGround (const csm::ImageCoord &image_pt,
                                       double height, double desired_precision,
                                       double *achieved_precision,
                                       csm::WarningList *warnings) const
{
  MESSAGE_LOG (
  ...
  double xl, yl, zl;
  double dxl, dyl, dzl;
  losToEcf (image_pt.line, image_pt.samp, _no_adjustment, xc, yc, zc, vx, vy,
            vz, xl, yl, zl);
...
{
  MESSAGE_LOG (
      "Calculating determineSensorCovarianceInImageSpace for {} {} {}", gp.x,
      gp.y, gp.z)
  ...
  for (i = 0; i < totalAdjParams; i++)
    {
      for (j = 0; j < totalAdjParams; j++)
        {
          sensor_cov[0] += sensor_partials[i].first
                           * getParameterCovariance (i, j)
                           * sensor_partials[j].first;
          sensor_cov[1] += sensor_partials[i].second
                           * getParameterCovariance (i, j)
                           * sensor_partials[j].first;
          sensor_cov[2] += sensor_partials[i].first
                           * getParameterCovariance (i, j)
    ...
    csm::WarningList *warnings) const
{
  ...
  csm::EcefCoord gp1 = imageToGround (image_pt, height, desired_precision,
                                      achieved_precision);
...
csm::RasterGM::SensorPartials
UsgsAstroLsSensorModel::computeSensorPartials (
    int index, const csm::ImageCoord &image_pt,
    const csm::EcefCoord &ground_pt, double desired_precision,
    double *achieved_precision, csm::WarningList *warnings) const
{
  MESSAGE_LOG ("Calculating computeSensorPartials (with image points {}, {}) "
               "for ground "
               "point {}, {}, {} with desired precision {}",
               image_pt.line, image_pt.samp, ground_pt.x, ground_pt.y,
               ground_pt.z, desired_precision)
    ...
    csm::WarningList *warnings) const
{
  MESSAGE_LOG (
      ...
      "desired precision {}",
      ground_pt.x, ground_pt.y, ground_pt.z, desired_precision)
  csm::ImageCoord image_pt = groundToImage (ground_pt, desired_precision,
                                            achieved_precision, warnings);

  return computeAllSensorPartials (image_pt, ground_pt, pSet,
                                   desired_precision, achieved_precision,
                                   warnings);
}
  ...
  for (int index = 0; index < num; index++)
    {
      partials.push_back (computeSensorPartials (
          indices[index], image_pt, ground_pt, desired_precision,
          achieved_precision, warnings));
    }
  return partials;
}
...
//***************************************************************************
void
UsgsAstroLsSensorModel::setParameterCovariance (int index1, int index2,
                                                double covariance)
{
  int index = UsgsAstroLsSensorModel::NUM_PARAMETERS * index1 + index2;
    ...
    const csm::EcefCoord &groundPt) const
{
  ...
  csm::EcefVector illuminationDirection = csm::EcefVector (
      groundPt.x - sunPosition.x, groundPt.y - sunPosition.y,
...
//***************************************************************************
void
UsgsAstroLsSensorModel::setGeometricCorrectionSwitch (int index, bool value,
    ...
    double attCorr[9]) const
{
  ...
  euler[2] = (getValue (8, adj) + getValue (11, adj) * nTime
             ...
             / m_halfSwath;
  MESSAGE_LOG ("calculateAttitudeCorrection: euler {} {} {}", euler[0],
               euler[1], euler[2])
    ...
    const std::vector<double> &adj) const
{
  ...
  MESSAGE_LOG ("computeDetectorView: look vector {} {} {}", bodyLookX,
               bodyLookY, bodyLookZ)
    ...
    const std::string imageSupportData, csm::WarningList *warnings)
{
  ...
  for (int i = 0; i < ephemTime.size (); i++)
    {
      rotatedSunState
          = j2000_to_target.rotateStateAt (ephemTime[i], sunStates[i]);
  ...
  for (int i = 0; i < ephemTime.size (); i++)
    {
      rotatedInstState = j2000_to_target.rotateStateAt (
          ephemTime[i], instStates[i], ale::SLERP);
      // ALE operates in Km and we want m
...
UsgsAstroLsSensorModel::getSunPosition (const double imageTime) const
{
  ...
  if ((numSunPositions / 3) > 1)
  ...
  else if ((numSunVelocities / 3) >= 1)
    {
      ...
      //  then the illumination direction is calculated via linear
      //  extrapolation.
