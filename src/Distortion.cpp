    ...
    DistortionType distortionType, const double tolerance)
{
    ...
    case KAGUYALISM: {
        ...
        if (opticalDistCoeffs.size() != 10) {
            ...
            std::string message = "Distortion coefficients for Kaguya LISM must be of size 10, "
                                  "got: "
    ...
    case LROLROCNAC: {
        if (opticalDistCoeffs.size() != 1) {
            csm::Error::ErrorType errorType = csm::Error::INDEX_OUT_OF_RANGE;
            std::string message = "Distortion coefficients for LRO LROC NAC must be of size 1, "
                                  "current size: "
                + std::to_string(opticalDistCoeffs.size());
            std::string function = "removeDistortion";
            throw csm::Error(errorType, message, function);
        ...
        if (den == 0.0) {
            ...
            std::string message = "Unable to remove distortion for LRO LROC NAC. Focal plane "
                                  "position "
                + std::to_string(dy);
            std::string function = "removeDistortion";
            throw csm::Error(errorType, message, function);
    ...
    const double desiredPrecision, const double tolerance)
{
    ...
    case RADIAL: {
        ...
        if (rp2 > tolerance) {
            ...
            double drOverR = opticalDistCoeffs[0] + (rp2 * (opticalDistCoeffs[1] + (rp2 * opticalDistCoeffs[2])));
            ...
            do {
                ...
                drOverR = opticalDistCoeffs[0] + (r2_prev * (opticalDistCoeffs[1] + (r2_prev * opticalDistCoeffs[2])));
