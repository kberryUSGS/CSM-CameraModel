...
void
distortionJacobian (double x, double y, double *jacobian,
 ...
 */
void
computeTransverseDistortion (double ux, double uy, double &dx, double &dy,
                             const std::vector<double> opticalDistCoeffs)
{
  double f[10];
  f[0] = 1;
...
void
removeDistortion (double dx, double dy, double &ux, double &uy,
                  const std::vector<double> opticalDistCoeffs,
                  DistortionType distortionType, const double tolerance)
{
  ...
  switch (distortionType)
        ...
        if (opticalDistCoeffs.size () != 10)
          {
            ...
            std::string message = "Distortion coefficients for Kaguya LISM "
                                  "must be of size 10, "
                                  "got: "
    ...
    // the remove function iteratively computes undistorted coordinates based
    // on the distorted coordinates, rather than iteratively computing
    // distorted coordinates to undistorted coordinates.
    case DAWNFC:
      {
        double r2;
        int numAttempts = 1;
        ...
        do
          {
            ...
            if (abs (guess_dx - dx) < tolerance
                && abs (guess_dy - dy) < tolerance)
              {
                done = true;
              }
    ...
    case LROLROCNAC:
      {
        if (opticalDistCoeffs.size () != 1)
          {
            csm::Error::ErrorType errorType = csm::Error::INDEX_OUT_OF_RANGE;
            std::string message = "Distortion coefficients for LRO LROC NAC "
                                  "must be of size 1, "
                                  "current size: "
                                  + std::to_string (opticalDistCoeffs.size ());
            std::string function = "removeDistortion";
        ...
        double den
            = 1
              + dk1 * dy
                    * dy; // r = dy*dy = distance from the focal plane center
        if (den == 0.0)
          {
...
void
applyDistortion (double ux, double uy, double &dx, double &dy,
                 const std::vector<double> opticalDistCoeffs,
                 DistortionType distortionType, const double desiredPrecision,
                 const double tolerance)
{
  ...
  switch (distortionType)
    ...
    // until the new distorted point, r, undistorts to within a tolerance of
    // the original point, rp.
    case RADIAL:
      {
        ...
        if (rp2 > tolerance)
          {
            ...
            double drOverR
                = opticalDistCoeffs[0]
                  + (rp2
                     * (opticalDistCoeffs[1] + (rp2 * opticalDistCoeffs[2])));
            ...
            do
              {
                ...
                // undistorted position. Also, make sure the focal plane is
                // less than 1km, it is unreasonable for it to grow larger than
                // that.
    ...
    case LROLROCNAC:
      {
        ...
        // 116.881145553046 then there is no root to find.  Further, the
        // greatest y that any measure on the sensor will acutally distort to
        // is less than 20.  Thus, if any distorted measure is greater that
        // that skip the iterations.  The points isn't in the cube, and exactly
        // how far outside the cube is irrelevant.  Just let the camera model
        // know its not in the cube....
        if (fabs (uy) > 40)
