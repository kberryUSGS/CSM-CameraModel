    ...
    csm::WarningList *warnings) const
{
  ...
  computePixel (distortedX, distortedY, m_ccdCenter[1], m_ccdCenter[0],
                ...
                &m_iTransS[0], &m_iTransL[0], line, sample);
    ...
    csm::WarningList *warnings) const
{
  ...
  MESSAGE_LOG ("Computing imageToProximateImagingLocus(No ground) for point "
               "{}, {}, {}, "
    ...
    double *achievedPrecision, csm::WarningList *warnings) const
{
  ...
  std::vector<double> lookBUnit{ lookB[0] / mag, lookB[1] / mag,
                                 lookB[2] / mag };

  return csm::EcefLocus (
      m_currentParameterValue[0], m_currentParameterValue[1],
      m_currentParameterValue[2], lookBUnit[0], lookBUnit[1], lookBUnit[2]);
}
...
{
  // ground (body-fixed) - sun (body-fixed) gives us the illumination
  // direction.
    ...
    csm::WarningList *warnings) const
{
  ...
  for (int index = 0; index < num; index++)
    {
      partials.push_back (computeSensorPartials (indices[index], imagePt,
                                                 groundPt, desiredPrecision,
                                                 achievedPrecision, warnings));
    }
  return partials;
}
    ...
    csm::WarningList *warnings) const
{
  MESSAGE_LOG (
      "Computing all sensor partials image point from ground point {}, {}, {} \
                               and desiredPrecision: {}",
      groundPt.x, groundPt.y, groundPt.z, desiredPrecision);
  csm::ImageCoord imagePt = groundToImage (groundPt, desiredPrecision,
                                           achievedPrecision, warnings);
...
UsgsAstroFrameSensorModel::getModelState () const
{
  ...
  json state
      = { { "m_modelName", _SENSOR_MODEL_NAME },
          ...
          { "m_boresight",
            { m_boresight[0], m_boresight[1], m_boresight[2] } },
  ...
  try
    {
      ...
      m_startingDetectorLine
          = state.at ("m_startingDetectorLine").get<double> ();
      m_detectorSampleSumming
          = state.at ("m_detectorSampleSumming").get<double> ();
      m_detectorLineSumming
          = state.at ("m_detectorLineSumming").get<double> ();
      ...
      m_distortionType
          = (DistortionType)state.at ("m_distortionType").get<int> ();
      m_opticalDistCoeffs
      ...
      m_currentParameterCovariance = state.at ("m_currentParameterCovariance")
                                                  ...
                                                  csm::WarningList *warnings)
{
  ...
  state["m_startingDetectorSample"]
      = ale::getDetectorStartingSample (parsedIsd);
  ...
  try
    {
      state["m_focalLengthEpsilon"]
          = ale::getFocalLengthUncertainty (parsedIsd);
    }
  ...
  try
    {
      ...
      state["m_transX"][1]
          = state["m_iTransL"][1].get<double> () / determinant;
      state["m_transX"][2]
          = -state["m_iTransS"][1].get<double> () / determinant;
      ...
      state["m_transY"][1]
          = -state["m_iTransL"][2].get<double> () / determinant;
      state["m_transY"][2]
          = state["m_iTransS"][2].get<double> () / determinant;
...
double
UsgsAstroFrameSensorModel::getParameterCovariance (int index1,
                                                   int index2) const
{
...
void
UsgsAstroFrameSensorModel::setParameterCovariance (int index1, int index2,
                                                   double covariance)
{
