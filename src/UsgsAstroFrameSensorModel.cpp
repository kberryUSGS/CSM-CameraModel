...
void
UsgsAstroFrameSensorModel::reset()
{
 ...
 */
csm::ImageCoord
UsgsAstroFrameSensorModel::groundToImage(const csm::EcefCoord& groundPt,
...
csm::ImageCoordCovar
UsgsAstroFrameSensorModel::groundToImage(const csm::EcefCoordCovar& groundPt,
                                         double desiredPrecision,
                                         double* achievedPrecision,
                                         csm::WarningList* warnings) const
{
...
csm::EcefCoord
UsgsAstroFrameSensorModel::imageToGround(const csm::ImageCoord& imagePt,
                                         double height,
                                         double desiredPrecision,
                                         double* achievedPrecision,
                                         csm::WarningList* warnings) const
{
  ...
  computeDistortedFocalPlaneCoordinates(line,
                                        sample,
                                        m_ccdCenter[1],
                                        m_ccdCenter[0],
                                        m_detectorSampleSumming,
  ...
  removeDistortion(x_camera,
                   ...
                   m_distortionType);
  MESSAGE_LOG(
    "Found undistortedX: {}, and undistortedY: {}", undistortedX, undistortedY);

  // Now back from distorted mm to pixels
  xl =
    m[0][0] * undistortedX + m[0][1] * undistortedY - m[0][2] * -m_focalLength;
  yl =
    m[1][0] * undistortedX + m[1][1] * undistortedY - m[1][2] * -m_focalLength;
  zl =
    m[2][0] * undistortedX + m[2][1] * undistortedY - m[2][2] * -m_focalLength;
  ...
  MESSAGE_LOG("Set xc, yc, zc to {}, {}, {}",
              m_currentParameterValue[0],
              m_currentParameterValue[1],
              m_currentParameterValue[2]);
...
csm::EcefCoordCovar
UsgsAstroFrameSensorModel::imageToGround(const csm::ImageCoordCovar& imagePt,
                                         double height,
...
csm::EcefLocus
UsgsAstroFrameSensorModel::imageToProximateImagingLocus(
...
csm::EcefLocus
UsgsAstroFrameSensorModel::imageToRemoteImagingLocus(
  const csm::ImageCoord& imagePt,
  double desiredPrecision,
  double* achievedPrecision,
  csm::WarningList* warnings) const
{
  ...
  computeDistortedFocalPlaneCoordinates(imagePt.line,
                                        imagePt.samp,
                                        m_ccdCenter[1],
                                        m_ccdCenter[0],
                                        m_detectorSampleSumming,
  ...
  double m[3][3];
  calcRotationMatrix(m);
  std::vector<double> lookC{ undistortedFocalPlaneX,
                             undistortedFocalPlaneY,
                             m_focalLength };
  ...
  std::vector<double> lookBUnit{ lookB[0] / mag,
                                 lookB[1] / mag,
                                 lookB[2] / mag };

  return csm::EcefLocus(m_currentParameterValue[0],
                        m_currentParameterValue[1],
                        m_currentParameterValue[2],
                        lookBUnit[0],
                        lookBUnit[1],
...
csm::ImageCoord
UsgsAstroFrameSensorModel::getImageStart() const
{
...
csm::ImageVector
UsgsAstroFrameSensorModel::getImageSize() const
{
  MESSAGE_LOG(
    "Accessing Image Size line: {}, sample: {}", m_nLines, m_nSamples);
...
UsgsAstroFrameSensorModel::getValidImageRange() const
{
  ...
  MESSAGE_LOG("Valid image range: min {}, {} max: {}, {}",
              min_pt.samp,
              min_pt.line,
              max_pt.samp,
              max_pt.line)
...
UsgsAstroFrameSensorModel::getValidHeightRange() const
{
  MESSAGE_LOG(
    "Accessing Image Height min: {}, max: {}", m_minElevation, m_maxElevation);
  return std::pair<double, double>(m_minElevation, m_maxElevation);
}

csm::EcefVector
UsgsAstroFrameSensorModel::getIlluminationDirection(
...
double
UsgsAstroFrameSensorModel::getImageTime(const csm::ImageCoord& imagePt) const
{
  MESSAGE_LOG(
    "Accessing image time for image point {}, {}", imagePt.line, imagePt.samp);
...
csm::EcefCoord
UsgsAstroFrameSensorModel::getSensorPosition(double time) const
{
  ...
  const csm::ImageCoord& imagePt) const
{
  ...
  return csm::EcefVector{ m_spacecraftVelocity[0],
                          m_spacecraftVelocity[1],
                          m_spacecraftVelocity[2] };
}

csm::EcefVector
UsgsAstroFrameSensorModel::getSensorVelocity(double time) const
{
  MESSAGE_LOG("Accessing sensor position for time {}", time);
  if (time == 0.0) {
    return csm::EcefVector{ m_spacecraftVelocity[0],
  ...
  csm::WarningList* warnings) const
{
  ...
  return computeSensorPartials(
    index, img_pt, groundPt, desiredPrecision, achievedPrecision);
}
  ...
  csm::WarningList* warnings) const
{
  ...
  for (int index = 0; index < num; index++) {
    partials.push_back(computeSensorPartials(indices[index],
                                             imagePt,
                                             groundPt,
                                             desiredPrecision,
                                             achievedPrecision,
...
std::vector<double>
UsgsAstroFrameSensorModel::computeGroundPartials(
  const csm::EcefCoord& groundPt) const
{
...
UsgsAstroFrameSensorModel::getCorrelationModel() const
{
...
csm::Version
UsgsAstroFrameSensorModel::getVersion() const
{
...
std::string
UsgsAstroFrameSensorModel::getModelName() const
{
...
std::string
UsgsAstroFrameSensorModel::getPedigree() const
{
...
std::string
UsgsAstroFrameSensorModel::getImageIdentifier() const
{
...
void
UsgsAstroFrameSensorModel::setImageIdentifier(const std::string& imageId,
...
std::string
UsgsAstroFrameSensorModel::getSensorIdentifier() const
{
...
std::string
UsgsAstroFrameSensorModel::getPlatformIdentifier() const
{
...
std::string
UsgsAstroFrameSensorModel::getCollectionIdentifier() const
{
...
std::string
UsgsAstroFrameSensorModel::getTrajectoryIdentifier() const
{
...
std::string
UsgsAstroFrameSensorModel::getSensorType() const
{
...
std::string
UsgsAstroFrameSensorModel::getSensorMode() const
{
...
std::string
UsgsAstroFrameSensorModel::getReferenceDateAndTime() const
{
...
std::string
UsgsAstroFrameSensorModel::getModelState() const
{
  ...
  json state = {
    ...
    { "m_spacecraftVelocity",
      { m_spacecraftVelocity[0],
        ...
        m_spacecraftVelocity[2] } },
    { "m_sunPosition",
      { m_sunPosition[0], m_sunPosition[1], m_sunPosition[2] } },
...
bool
UsgsAstroFrameSensorModel::isValidModelState(const std::string& stringState,
                                             csm::WarningList* warnings)
{
  ...
  if (!missingKeywords.empty() && warnings) {
    ...
    std::copy(missingKeywords.begin(),
              missingKeywords.end(),
...
bool
UsgsAstroFrameSensorModel::isValidIsd(const std::string& Isd,
                                      csm::WarningList* warnings)
{
...
void
UsgsAstroFrameSensorModel::replaceModelState(const std::string& stringState)
{
...
std::string
UsgsAstroFrameSensorModel::constructStateFromIsd(const std::string& jsonIsd,
                                                 csm::WarningList* warnings)
{
  ...
  try {
    ...
    state["m_transX"][0] = -(
      state["m_transX"][1].get<double>() * state["m_iTransL"][0].get<double>() +
    ...
    state["m_transY"][0] = -(
      state["m_transY"][1].get<double>() * state["m_iTransL"][0].get<double>() +
      state["m_transY"][2].get<double>() * state["m_iTransS"][0].get<double>());
  } catch (...) {
    MESSAGE_LOG("Could not compute detector pixel to focal plane coordinate "
                "transformation.");
    parsingWarnings->push_back(
      csm::Warning(csm::Warning::DATA_NOT_AVAILABLE,
  ...
  if (!parsingWarnings->empty()) {
    if (warnings) {
      warnings->insert(
        warnings->end(), parsingWarnings->begin(), parsingWarnings->end());
    }
...
csm::EcefCoord
UsgsAstroFrameSensorModel::getReferencePoint() const
{
...
void
UsgsAstroFrameSensorModel::setReferencePoint(const csm::EcefCoord& groundPt)
{
...
int
UsgsAstroFrameSensorModel::getNumParameters() const
{
...
std::string
UsgsAstroFrameSensorModel::getParameterName(int index) const
{
...
std::string
UsgsAstroFrameSensorModel::getParameterUnits(int index) const
{
...
bool
UsgsAstroFrameSensorModel::hasShareableParameters() const
{
...
bool
UsgsAstroFrameSensorModel::isParameterShareable(int index) const
{
...
csm::SharingCriteria
UsgsAstroFrameSensorModel::getParameterSharingCriteria(int index) const
{
...
double
UsgsAstroFrameSensorModel::getParameterValue(int index) const
{
  MESSAGE_LOG("Accessing parameter value {} at index: {}",
              m_currentParameterValue[index],
              index);
  return m_currentParameterValue[index];
}

void
UsgsAstroFrameSensorModel::setParameterValue(int index, double value)
{
...
csm::param::Type
UsgsAstroFrameSensorModel::getParameterType(int index) const
{
  MESSAGE_LOG(
    "Accessing parameter type: {} at index: {}", m_parameterType[index], index);
  return m_parameterType[index];
}

void
UsgsAstroFrameSensorModel::setParameterType(int index, csm::param::Type pType)
{
...
double
UsgsAstroFrameSensorModel::getParameterCovariance(int index1, int index2) const
{
  ...
  MESSAGE_LOG("Accessing parameter covar: {} between index1: {} and index2: {}",
              m_currentParameterCovariance[index],
              index1,
              index2);
...
void
UsgsAstroFrameSensorModel::setParameterCovariance(int index1,
                                                  int index2,
                                                  double covariance)
{
...
int
UsgsAstroFrameSensorModel::getNumGeometricCorrectionSwitches() const
{
...
std::string
UsgsAstroFrameSensorModel::getGeometricCorrectionName(int index) const
{
  MESSAGE_LOG(
...
void
UsgsAstroFrameSensorModel::setGeometricCorrectionSwitch(int index,
                                                        bool value,
...
bool
UsgsAstroFrameSensorModel::getGeometricCorrectionSwitch(int index) const
{
  MESSAGE_LOG(
...
csm::Ellipsoid
UsgsAstroFrameSensorModel::getEllipsoid() const
{
...
void
UsgsAstroFrameSensorModel::setEllipsoid(const csm::Ellipsoid& ellipsoid)
{
  MESSAGE_LOG("Setting ellipsoid radii {} {}",
              ellipsoid.getSemiMajorRadius(),
...
void
UsgsAstroFrameSensorModel::calcRotationMatrix(double m[3][3]) const
{
...
void
UsgsAstroFrameSensorModel::calcRotationMatrix(
  double m[3][3],
...
void
UsgsAstroFrameSensorModel::losEllipsoidIntersect(const double height,
                                                 const double xc,
                                                 const double yc,
                                                 const double zc,
                                                 const double xl,
...
double
UsgsAstroFrameSensorModel::getValue(
  int index,
  const std::vector<double>& adjustments) const
{
  MESSAGE_LOG("Accessing value: {} at index: {}, with adjustments",
              m_currentParameterValue[index] + adjustments[index],
              index);
  return m_currentParameterValue[index] + adjustments[index];
}

std::shared_ptr<spdlog::logger>
UsgsAstroFrameSensorModel::getLogger()
{
  return m_logger;
}

void
UsgsAstroFrameSensorModel::setLogger(std::string logName)
{
