...
public:
  ...
  imageToGround (const csm::ImageCoord &imagePt, double height = 0.0,
                 double desiredPrecision = 0.001,
                 double *achievedPrecision = NULL,
                 csm::WarningList *warnings = NULL) const;
  ...
  /**
   * Calculates the illumination vector (body-fixed, meters) from the sun to
   * the given ground point.
   *
   ...
   * @param imagePt Image coordinate to find the sensor position for.
   *
   * @return @b csm::EcefVector Returns the sensor velocity in body-fixed
   * frame.
   *
   * @throw csm::Error::BOUNDS "Image coordinate () out of bounds."
   */
  ...
  virtual std::vector<double>
  getCrossCovarianceMatrix (const GeometricModel &comparisonModel,
                            csm::param::Set pSet = csm::param::VALID,
                            const GeometricModelList &otherModels
                            = GeometricModelList ()) const;
