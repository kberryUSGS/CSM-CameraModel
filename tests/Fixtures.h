...
inline void
jsonToIsd (nlohmann::json &object, csm::Isd &isd, std::string prefix = "")
{
  for (nlohmann::json::iterator it = object.begin (); it != object.end ();
       ++it)
    {
    ...
    if (sensorModel)
      {
        ...
        std::uintptr_t sensorId
            = reinterpret_cast<std::uintptr_t> (sensorModel);
...
protected:
  ...
  createModel (csm::Isd &modifiedIsd)
  {
    UsgsAstroPlugin frameCameraPlugin;
    csm::Model *model = frameCameraPlugin.constructModelFromISD (
...
protected:
  csm::Isd isd;
  UsgsAstroFrameSensorModel *
  createModifiedStateSensorModel (std::string key, double newValue)
  {
