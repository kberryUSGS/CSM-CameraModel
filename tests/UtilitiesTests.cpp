...
TEST (UtilitiesTests, calculateRotationMatrixFromEuler)
{
  ...
  // EXPECT_NEARs are used here instead of EXPECT_DOUBLE_EQs because index 0
  // and 8 of the matrix are evaluating to 6.12...e-17. This is too small to be
...
TEST (UtilitiesTests, computePixel)
{
  ...
  double iTransL[] = { 0.0, 10.0, 0.0 };
  double line, sample;
  computePixel (0.0, -0.4, 8.0, 0.5, 1.0, 1.0, 0.0, 0.0, iTransS, iTransL,
                line, sample);
...
TEST (UtilitiesTests, computePixelSumming)
{
  ...
  double iTransL[] = { 0.0, 10.0, 0.0 };
  double line, sample;
  computePixel (-0.4, 0.0, 8.0, 8.0, 2.0, 2.0, 0.0, 0.0, iTransS, iTransL,
                line, sample);
...
TEST (UtilitiesTests, computePixelStart)
{
  ...
  double iTransL[] = { 0.0, 10.0, 0.0 };
  double line, sample;
  computePixel (-0.5, -0.2, 8.0, 8.0, 1.0, 1.0, 2.0, 1.0, iTransS, iTransL,
                line, sample);
  EXPECT_DOUBLE_EQ (line, 2.0);
  EXPECT_DOUBLE_EQ (sample, 4.0);
}
...
TEST (UtilitiesTests, computePixelStartSumming)
{
  ...
  double iTransL[] = { 0.0, 10.0, 0.0 };
  double line, sample;
  computePixel (-0.5, -0.2, 8.0, 8.0, 2.0, 4.0, 2.0, 1.0, iTransS, iTransL,
                line, sample);
  EXPECT_DOUBLE_EQ (line, 0.5);
...
{
  std::function<double(double)> testPoly
      = [](double x) { return (x - 2) * (x + 1) * (x + 7); };
...
{
  std::vector<double> oneRootCoeffs
      = { -12.0, 4.0, -3.0, 1.0 };                      // roots are 3, +-2i
